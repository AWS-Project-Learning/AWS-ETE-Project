  ...
  switch (action) {
    ...
    case 'remediate':
      return `Assessed ${res.assessed ?? 0} finding(s): ${res.auto_patch ?? 0} auto-patch, ${res.escalate ?? 0} escalate. ` +
             `Patch + deploy steps are proposed — review them in agent.tools, then confirm to execute.`
    case 'draft':
...
function ReportCard({ text }) {
  ...
  return (
    <div style={{
      ...
      <p style={{ margin: 0, padding: '8px 10px', fontSize: 11, color: '#d1fae5', whiteSpace: 'pre-wrap', lineHeight: 1.55 }}>
        {text}
      </p>
    </div>
  )
...
export default function AgentConsole({ service = 'bff', tools, scanId, recordId }) {
  ...
  const runAction = async (action, label) => {
    ...
    const evId = uid()
    tools?.startEvent({ id: evId, ts: ts(), title: label })
    try {
      const res = await triggerAgentAction({ action, service: svc })
      const r = res.result || {}
  ...
  const sendMessage = async (text, extra = {}) => {
    ...
    try {
      const res = await triggerChat({
        ...
        service: svc, scan_id: scanId || undefined, record_id: recordId || undefined,
        page: 'scan', ...extra,
      })
      const r = res.result || {}
      await streamTrace(evId, r.trace)
  ...
  const confirmPending = (idx, pending) => {
    setMessages(prev => prev.map((m, i) => (i === idx ? { ...m, pending: null } : m)))
    sendMessage('', { confirm_action: pending.token })
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          width: 30, height: 30, borderRadius: 9, background: 'rgba(99,102,241,0.25)',
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 9.5, color: '#64748b' }}>
            target: <span style={{ color: '#a5b4fc' }}>{svc}</span> · watch tools run in agent.tools →
          </p>
        </div>
      ...
      <div style={{
        ...
        {messages.length === 0 && (
          <div style={{ color: '#475569', fontSize: 12, lineHeight: 1.6, margin: 'auto', textAlign: 'center', maxWidth: 280 }}>
            <Sparkles size={20} color="#475569" style={{ marginBottom: 8 }} />
            <p style={{ margin: 0 }}>Pick an action above, or ask a question.</p>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{
            ...
            maxWidth: '88%',
            background: m.role === 'user' ? 'rgba(59,130,246,0.15)'
              : m.error ? 'rgba(239,68,68,0.08)' : 'rgba(15,23,42,0.85)',
      ...
      <form onSubmit={e => { e.preventDefault(); sendMessage() }}
        ...
        <input value={input} onChange={e => setInput(e.target.value)} disabled={busy}
          placeholder="Ask about scans, health, findings…"
